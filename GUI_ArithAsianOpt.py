...
def mainCal(typ, asset_price, mature_time, strike_price, rate, vol, n, path, ctl_var):
...
text7 = tk.Label(win, text = 'Ctl Variable').grid(row=7) 
ctl_var = tk.BooleanVar()
check7 = ttk.Checkbutton(win, variable=ctl_var).grid(row=7, column=1)
ctl_var.set(True)

text8 = tk.Label(win, text = 'Monte Carl Path: ').grid(row=8) 
path = tk.StringVar()
...
button = tk.ttk.Button(win, text="Calculate", command=lambda : mainCal(typex.get(), priceA.get(), timeM.get(), priceK.get(), rate.get(), vol.get(), n.get(), path.get(), ctl_var.get()))

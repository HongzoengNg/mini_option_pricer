...
def mainCal(typ, asset_price1, asset_price2, mature_time, strike_price, rate, vol1, vol2, rol, path, ctl_var):
    if typ == 'Call Option':
        tp = 'c'
    elif typ == 'Put Option':
        tp = 'p'
    ...
    #future_type, asset_price1, asset_price2,mature_time,strike_price,rate,vol1,vol2,rol
    abopx = abop.aritbasket_opt(tp, asset_price1, asset_price2, mature_time, strike_price, rate, vol1, vol2, rol)
    value = abopx.value()
...
text9 = tk.Label(win, text = 'Ctl Variable').grid(row=9) 
ctl_var = tk.BooleanVar()
check9 = ttk.Checkbutton(win, variable=ctl_var).grid(row=9, column=1)
ctl_var.set(True)


text10 = tk.Label(win, text = 'Monte Carl Path: ').grid(row=10) 
path = tk.StringVar()

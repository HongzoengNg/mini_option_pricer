...
def mainCal(typ, asset_price, strike_price, mature_time, vol, rate, repo_q, st):
...
def main():
    ...
    text7 = tk.Label(win, text = 'Steps: ').grid(row=7) 
    st = tk.StringVar()
    entry7 = tk.ttk.Entry(win, textvariable = st)
    entry7.grid(row=7, column=1)


    text8 = tk.Label(win, text = 'Price: ').grid(row=8) 
    text9 = tk.Label(win)
    text9.grid(row=8, column=1)


    button = tk.ttk.Button(win, text="Calculate", command=lambda : mainCal(typex.get(), priceA.get(), priceK.get(), timeM.get(), vol.get(), rate.get(), repoR.get(), st.get()))
    button.grid()
